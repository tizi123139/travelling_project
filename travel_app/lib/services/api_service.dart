...
class ApiService {
  ...
  static Map<String, String> _headers = {
    ...
    static Future<Map<String, dynamic>> login({
      ...
      if (responseData['code'] == 200 || response.statusCode == 200) {
      final tokenData = responseData['data'] ?? {};
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString('access_token', tokenData['access_token'] ?? '');
      await prefs.setString('username', tokenData['username'] ?? username);
      await prefs.setInt('expires_at', tokenData['expires_at'] ?? 0);
      
      // 返回与 login_page.dart 匹配的结构
      return {
        'code': 200,
        'message': '登录成功',
        'data': tokenData,
...
static Future<Map<String, dynamic>> sendSmsCode({
    ...
    try {
      final response = await http.post(
        Uri.parse("$baseUrl/users/send-sms-code"),
