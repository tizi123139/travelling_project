...
class _FoodPageState extends State<FoodPage> {
 ...
 void _showFoodDetail(Food food) async {
  // 先加载后端详情数据
  final Food? detailFood = await _foodService.getFoodDetail(food.id);
  final showFood = detailFood ?? food; // 兜底用本地数据
          ...
          children: [
            ...
            Row(
              children: [
                ...
                Text(' ${showFood.rating.toStringAsFixed(1)}', style: TextStyle(
                  color: isDark ? const Color(0xFFE0E0E0) : const Color(0xFF333333),
                )),
                Text(' (${showFood.reviewCount}条评价)', style: TextStyle(
                  color: Colors.grey,
                )),
                const Spacer(),
                Text('¥${showFood.price}', style: TextStyle(
                  fontSize: 20,
                  fontWeight: FontWeight.bold,
            ...
            Wrap(
              spacing: 8,
              children: showFood.tags.map((tag) {
                return Chip(
                  label: Text(tag),
                  backgroundColor: isDark ? const Color(0xFF2D2D2D) : Colors.grey.shade200,
                );
            ...
            // 地址和营业时间
            _buildInfoRow(Icons.location_on, '地址：${showFood.address}', isDark),
            _buildInfoRow(Icons.access_time, '营业时间：${showFood.businessHours}', isDark),
            _buildInfoRow(Icons.phone, '电话：${showFood.phone}', isDark),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed: () => _navigateToRestaurant(showFood),
