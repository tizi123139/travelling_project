...
class CultureService {
  static const String baseUrl = 'http://localhost:8000/api/culture';

  Future<List<CultureItem>> getAllCultureItems({String destination = "武汉"}) async {
    try {
      final response = await http.get(
        Uri.parse('$baseUrl/intangible/list?destination=$destination'),
      );
  ...
  Future<void> likeItem(String itemId) async {
    try {
      final response = await http.post(
        Uri.parse('$baseUrl/intangible/$itemId/like'),
      );
